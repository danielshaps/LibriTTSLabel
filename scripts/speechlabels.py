...
class Utterance(object):
    ...
    def _read_txt(cls, filepath, tiername='segment'):
        with open(filepath, encoding="utf-8") as infh:
    ...
    def _loadFromRec(self):
        ...
        with open(self.filepath, encoding="utf-8") as infh:
            recfile = infh.readlines()
    ...
    def saveLab(self, filepath=None):
        ...
        if filepath is None:
            fh = open(self.name + "." + Utterance.LAB_EXT, "w", encoding="utf-8")
        else:
    ...
    def saveTextgrid(self, filepath=None):
        ...
        if filepath is None:
            f = open(self.name + "." + Utterance.TEXTGRID_EXT, "w", encoding="utf-8")
        else:
...
#        tierkeys.sort()
    ...
    def dumpPhoneSequence(self, delim="\n", destdir=None, destfilename=None):
        ...
        with open(os.path.join(destdir, destfilename), "w", encoding="utf-8") as outfh:
            outfh.write(outstr)
...
class Corpus(object):
    ...
    def meanOVR(self, refname=None):
        ...
        count = 0
        for utt in self.utterances:
           ovrs = [x["ovr"] for x in utt.segment_comparisons[refname]]
           OVR_sum += sum(ovrs)
           count += len(ovrs)
    ...
    def writeOrthographicTranscriptionFile(self, outputfilelocation, stripword=False):
        ...
        with open(outputfilelocation, "w", encoding="utf-8") as outfh:
